...
export default function Editor({ 
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Update the user with initial feedback
      setStreamingOutput(prev => [...prev, "Analyzing prompt..."]);
      const processingWords = prompt.split(' ').slice(0, 3);
      processingWords.forEach(word => {
        if (word.length > 0) {
          setStreamingOutput(prev => [...prev, `Processing: ${word}`]);
        }
      });
        ...
        if (window.expectedContentLength && collectedHtml.length > 0) {
          ...
          setStreamingOutput(prev => {
            ...
            if (lastLine && lastLine.includes("Building HTML")) {
              ...
              return [...prev, `Building HTML... ${percent}% complete`];
            }
          });
        }
      }, 800);
            ...
            try {
              ...
              switch (event.event) {
                case 'start':
                  ...
                  setStreamingOutput(prev => [...prev, `AI Accelerate stream started`]);
