...
export default function Editor({ 
  ...
  const [apiConfig, setApiConfig] = useState<ApiConfig>(initialApiConfig || {
    provider: "Jatevo LLM Inference Router",
    apiKey: "",
    saveToken: false,
  });
  ...
  const handleGenerate = async (overridePrompt?: string, isFollowUp: boolean = false) => {
    ...
    setStreamingOutput(["Starting Jatevo LLM generation..."]);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setStreamingOutput(prev => [...prev, "Generating HTML with Jatevo..."]);
        }, 1500);
      }, 1000);
              ...
              switch (event.event) {
                ...
                case 'chunk':
                  ...
                  if (event.isHtml && !isHtmlStarted) {
                    ...
                    if (collectedHtml.length > 200) {
                      setStreamingOutput(prev => [...prev, `✅ Content successfully generated by Jatevo LLM!`]);
                ...
                case 'complete':
                  ...
                  if (event.tokenCount && (window as any).recordTokenUsage) {
                    ...
                    (window as any).recordTokenUsage(event.tokenCount, true);
                  }
                  ...
                  if (event.html) {
                    ...
                    setTimeout(() => {
                      if (event.source === "api") {
                        ...
                        setStreamingOutput(prev => [...prev, "✅ Content successfully generated by Jatevo LLM Inference!"]);
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: "✅ Content successfully generated by Jatevo LLM Inference!",
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-800 bg-[#0f172a]">
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          <h1 className="text-base md:text-lg font-semibold text-blue-400">Site<span className="text-white">Craft</span> 
            ...
            <span className={`${isMobile ? 'hidden' : 'inline'} text-xs text-gray-500 ml-2 font-normal`}>Powered by Jatevo LLM Inference</span>
          </h1>
        </div>
