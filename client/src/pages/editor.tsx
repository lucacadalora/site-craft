...
export default function Editor({ 
  ...
  const saveProjectToLocalStorage = useCallback((html: string, projectPrompt: string) => {
    if (html && projectPrompt) {
      const projectData = {
        html,
        prompt: projectPrompt,
        lastUpdated: new Date().toISOString(),
      };
      localStorage.setItem('savedLandingPage', JSON.stringify(projectData));
  ...
  useEffect(() => {
    const savedProject = localStorage.getItem('savedLandingPage');
    if (savedProject && !htmlContent) {
      try {
        const { html, prompt: savedPrompt } = JSON.parse(savedProject);
        if (html && savedPrompt) {
          setHtmlContent(html);
  ...
  // Start a new project (clear everything)
  const handleNewProject = () => {
    // Ask for confirmation
    if (htmlContent && prompt) {
      const confirmNew = window.confirm(
        "Starting a new project will clear your current work. Are you sure you want to continue?"
      );
    ...
    setPrompt("");
    setStreamingOutput([]);
    localStorage.removeItem('savedLandingPage');
    
    toast({
      title: "New Project Started",
      description: "You're now working on a fresh landing page",
    });
  };
  ...
  const handleGenerate = async () => {
    ...
    // Single status message - remove duplication
    setStreamingOutput(["Starting generation..."]);
    ...
    try {
      ...
      // Simplified status updates - no token estimates
      setStreamingOutput(prev => [...prev, "Processing your request..."]);
      await new Promise(resolve => setTimeout(resolve, 300));
      
      if (responseData.source === "api") {
        setStreamingOutput(prev => [...prev, "✅ Content successfully generated!"]);
      ...
      setStreamingOutput(prev => [...prev, "Building your landing page..."]);
      ...
      saveProjectToLocalStorage(htmlContent, prompt);
      
      toast({
        title: "Generation Complete",
        description: "Your landing page has been generated and saved",
      });
    } catch (error) {
    ...
    <div className="flex flex-col h-screen bg-[#111827] text-white">
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-800 bg-[#0f172a]">
        ...
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          >
            <FilePlus className="h-4 w-4 md:mr-1" />
          ...
          <Button 
            ...
            onClick={() => {
              ...
              toast({
                title: "Project Saved",
                description: "Your landing page has been saved locally",
              });
            }}
          >
            ...
            {!isGenerating ? (
              ...
              <div className="rounded-md overflow-hidden border border-blue-700 bg-[#1e293b] p-2">
                <div className="flex items-center mb-1">
                  ...
                  <h3 className="text-sm font-medium text-white">Generation Status</h3>
                </div>
